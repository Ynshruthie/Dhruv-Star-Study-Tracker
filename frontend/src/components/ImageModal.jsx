...
export const ImageModal = ({ isOpen, onClose, hourData, studentName }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-4 bg-slate-900/80 backdrop-blur-sm animate-in fade-in duration-150">
      <div className="relative w-full max-w-5xl bg-white border border-slate-200 rounded-xl sm:rounded-2xl shadow-2xl overflow-hidden flex flex-col h-[96vh] sm:h-[90vh]">
        
        {/* Modal Header */}
        <div className="px-3 sm:px-6 py-3 sm:py-4 bg-slate-50 border-b border-slate-200 flex items-center justify-between gap-2 shrink-0">
          <div className="min-w-0 flex items-center gap-2 sm:gap-3">
            ...
            <div className="min-w-0">
              <h3 className="text-base font-bold text-slate-900 flex items-center gap-2">
                <span className="truncate">{hourData.subject}</span>
                <span className="hidden sm:flex text-xs bg-emerald-50 text-emerald-700 border border-emerald-200 px-2.5 py-0.5 rounded-full items-center gap-1 font-medium shadow-sm">
                  <CheckCircle className="w-3.5 h-3.5 text-emerald-600" /> Verified Work
                </span>
              </h3>
              <p className="text-xs text-slate-500 flex items-center gap-2 mt-1 min-w-0">
                {studentName && <span className="truncate text-blue-700 font-semibold">{studentName}</span>}
        ...
        {/* Modal Footer */}
        <div className="px-3 sm:px-6 py-3 sm:py-4 bg-white border-t border-slate-200 flex items-center justify-between gap-2 text-xs shrink-0">
          <span className="hidden sm:flex items-center gap-1.5 font-mono text-slate-500 bg-slate-100 px-3 py-1.5 rounded-lg">
          ...
          {hasImages && (
            <a
              ...
              download
              className="ml-auto px-3 sm:px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg transition flex items-center gap-2 font-semibold text-xs sm:text-sm shadow-sm hover:shadow-md"
            >
              <Download className="w-4 h-4" />
              <span className="sm:hidden">Open</span><span className="hidden sm:inline">Open Original / Download</span>
            </a>
          )}
        </div>
